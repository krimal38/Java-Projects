...
class ThreeTenNetwork {
	/**
	 * This method add all nodes from the graph to the priority queue.
	 * 
	 * @param graph
	 *            - Calculates the shortes path.
	 * @param startNode
	 *            - Starts at the node and finds the shortest path from the start to
	 *            finish.
	 * @param queue
	 *            - Queue is to add values.
	 * @param hostToDijkstraNode
	 *            - HashTable that stores the information from the host.
	 */
	public static void setupDijkstras(Network graph, Host startNode, PriorityQueue<DijkstraNode> queue,
		...
		// for (Host hosting : graph.getVertices()) {

		for (int i = 0; i < graph.getVertices().size(); i++) {
			Collection<Host> collection = graph.getVertices();
			// obj of the DijkstraNode
			Host host = collection.iterator().next();
			collection.remove(host);
			DijkstraNode Hostnode = new DijkstraNode(host);
			// if the start node is the same as host then distance is 0
			if (host == startNode)
				Hostnode.distance = 0;
			else {
				// Storing routes on the queue
				queue.add(Hostnode);
				// adds on hashTable. Stores the routes information.
				hostToDijkstraNode.add(host, Hostnode);
			}
		}
	}

	/**
	 * This method sets up the routing table.
	 * 
	 * @param graph
	 *            - Adjacency list.
	 * @param startNode
	...
	public static void setupTables(Network graph, Host startNode, HashTable<Host, DijkstraNode> hostToDijkstraNode) {
		...
		// 3. Add the pair (other host, next hop) to the routing table for the start
		// node
		...
		for (Host hosting : graph.getVertices()) {
			DijkstraNode Node = hostToDijkstraNode.get(hosting);
			DijkstraNode otherNode = Node;

			while (Node != null) {
				if (Node.parent != null && Node.parent.host.equals(startNode) && Node.host != null) {
					startNode.getRoutingTable().add(otherNode.host, Node.host);
				}
				Node = Node.parent;
			}
		}
	...
	/**
	 * A node in Dijkstra's shortest path algothim needs to be able to be marked as
	 * done and have a parent and a current distance. This wraps a Host with those
	 * properties for running the algorithm.
	 * 
	 * @author Katherine (Raven) Russell
	 */
	private static class DijkstraNode implements Comparable<DijkstraNode> {
		...
		/**
		 * The "parent" which will be discovered by Dijkstra's algorithm.
		 */
		DijkstraNode parent;

		/**
		 * Whether or not the host is "done" (finalized) when running the algorithm.
		 */
		boolean done = false;

		/**
		 * The "distance" a node is from the starting node. Discovered by running
		 ...
		 *            the host to wrap
		 */
		public DijkstraNode(Host h) {
			this.host = h;
		}

		/**
		 * Compares two nodes in the algorithm so that nodes with the smallest
		 * "distance" are taken out of the priority queue first. Ties are broken by node
		 * id.
		 * 
		 * @param other
	...
	/**
	 * The current location of a message being sent in the network.
	 */
	...
	/**
	 * The default color of a node when it has a route, but isn't being used.
	 */
	...
	/**
	 * Runs Dijkstra's shortest path algorithm from a given starting node and
	 * constructs its routing table.
	 * 
	 * @param startNode
	 *            starting node for the algothim
	 */
	...
	/**
	 * Takes the first "step" when routing a new message from a source to a
	 * destination host in the network.
	 * 
	 * @param source
	 *            the source host of the message
	 * @param dest
	 *            the destination host of the message
	...
	/**
	 * Takes another "step" when routing a message to a destination host in the
	 * network.
	 * 
	 * @param dest
	 *            the destination host of the message
